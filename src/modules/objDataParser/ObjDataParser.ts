...
class ObjDataParser {
	...
	private m_vertices: number[] = null;
	private m_verticesLen: number = 0;
	private m_normals: number[] = null;
	private m_normalsLen: number = 0;
	private m_uvs: number[] = null;
	private m_uvsLen: number = 0;
	//
	private m_cachedRawNVS: number[] = null;
	private m_vtxTot: number = 0;
	// the raw data that is used to create Stage3d buffers
	private m_rawIVS: number[] = null;
	private m_rawVS: number[] = null;
	private m_rawUVS: number[] = null;
	private m_rawNVS: number[] = null;
	private m_rawCVS: number[] = null;
	// 用于拆分处理
	...
	parseStrData(objStr: string, scale: number, dataIsZxy: boolean = false, textureFlip: boolean = false): void {
		...
		this.m_verticesLen = 0;
		this.m_normals = null;
		this.m_normalsLen = 0;
		this.m_uvs = null;
		this.m_uvsLen = 0;
		this.m_cachedRawNVS = null;
		this.baseVertexIndex = 0;
		this.baseNormalIndex = 0;
		this.baseUvIndex = 0;
		...
		let lines = objStr.split(ObjDataParser.LINE_FEED);
	...
	private parseLine(line: string): void {
		...
		let words: string[] = line.split(ObjDataParser.SPACE);
		...
		switch (firstWord) {
			case ObjDataParser.VERTEX:
				this.parseVertex(data);
				break;
			case ObjDataParser.NORMAL:
				this.parseNormal(data);
				break;
			case ObjDataParser.UV:
				this.parseUV(data);
				break;
			case ObjDataParser.INDEX_DATA:
	...
	private parseIndex(data: string[]): void {
		...
		for (i = starthere; i < loop; ++i) {
			triplet = data[i];
			if (triplet == "") continue;
			subdata = triplet.split(ObjDataParser.SLASH);
 ...
 * 
 * */
class ObjsDataParser {
	constructor() { }

	private m_parserArr: ObjDataParser[] = [];
	parseStrData(objDataStr: string, scale: number = 1.0, dataIsZxy: boolean = false, textureFlip: boolean = false): void {
		...
		let i: number = 1;
		let parser: ObjDataParser = null;
		for (; i < list.length; ++i) {
			//trace("------------------------b-----b--------b---------------------");
			//trace("# object" + list[i]);
			parser = new ObjDataParser();
	...
	getParserAt(i: number): ObjDataParser {
		return this.m_parserArr[i];
	}

} // end class
export {ObjDataParser, ObjsDataParser}